...
1. サンプルコードをクローンし、
2. 目的のステップのブランチに切替え後に、
3. **vscode** の **「実行とデバッグ」** を開き、 **「Debug」** を選んで実行すると、
   1. ソースコードがビルドされ、
   2. プログラムがデバッグ実行されます。

[「○○くんのために一所懸命書いたものの結局○○くんの卒業に間に合わなかったGLFW による OpenGL 入門」](https://tokoik.github.io/GLFWdraft.pdf)の各章とサンプルコードのブランチとの対応は以下になります。
